...
- **Engine assets not precompiled in host apps using sprockets** (#61) — Resource index/show/edit pages 500'd on every host using `sprockets-rails` (Rails 7.1 default) with `Asset 'iron_admin/index.js' was not declared to be precompiled in production.`. The engine declared `precompile += %w[iron_admin_manifest]` but no `iron_admin_manifest.js` file existed in the gem. Added `app/assets/config/iron_admin_manifest.js` that `link_tree`s the engine's JS bundle, and the engine initializer now references the manifest with the correct extension. Apps on Rails 8 + propshaft were unaffected.
